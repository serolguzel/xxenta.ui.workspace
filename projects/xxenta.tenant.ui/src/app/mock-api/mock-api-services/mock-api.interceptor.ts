...
import { GENESIS_MOCK_API_DEFAULT_DELAY } from './mock-api.constants';
import { GenesisMockApiService } from './mock-api.service';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(GENESIS_MOCK_API_DEFAULT_DELAY);
    const genesisMockApiService = inject(GenesisMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = genesisMockApiService.findHandler(request.method.toUpperCase(), request.url);
