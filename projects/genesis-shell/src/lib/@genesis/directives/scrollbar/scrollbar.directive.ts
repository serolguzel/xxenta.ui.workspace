 ...
 */
@Directive({
    selector  : '[genesisScrollbar]',
    exportAs  : 'genesisScrollbar',
    standalone: true,
})
export class GenesisScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_genesisScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() genesisScrollbar: boolean = true;
    @Input() genesisScrollbarOptions: PerfectScrollbar.Options;

    private _animation?: number | null = null;
    private _options: PerfectScrollbar.Options;
    private _ps?: PerfectScrollbar | null | undefined = null;
    ...
    {
        // Enabled
        if ( 'genesisScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.genesisScrollbar = coerceBooleanProperty(changes.genesisScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.genesisScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'genesisScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.genesisScrollbarOptions.currentValue);
