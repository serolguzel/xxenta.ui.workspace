...
@Component({
  selector: 'pie-chart-task-route-type',
  templateUrl: './pie-chart-task-route-type.component.html',
  styleUrl: './pie-chart-task-route-type.component.scss',
  standalone: true,
  imports: [NgClass, ChartModule, ProgressSpinnerModule, TranslocoModule],
})
export class PieChartTaskRouteTypeComponent implements OnInit {
      ...
      datasets: [
        {
          ...
          backgroundColor: this.data.map((_, i) => PieChartTaskRouteTypeComponent.PALETTE[i % PieChartTaskRouteTypeComponent.PALETTE.length])
        }
      ]
    };
  }
